...
export const load: PageServerLoad = async ({ cookies }) => {
	...
	let events = appointment_data.calendar.map((appointment) => ({
		...
		end: Temporal.ZonedDateTime.from(unixToZonedDateTime(appointment.time + timeAddon)) //convert to Temporal.ZonedDateTime
	}));
...
function unixToZonedDateTime(unixSeconds: number, timeZone: string = "UTC"): string {
